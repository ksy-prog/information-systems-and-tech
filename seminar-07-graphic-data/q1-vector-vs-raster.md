...
## 5. Конвертация между форматами: вектор в растр и обратно

Важным аспектом работы с графикой является возможность конвертации одного типа в другой. Однако эти процессы имеют фундаментальные различия по сложности и результату.

### 5.1. Конвертация вектора в растр (растеризация)

Этот процесс технически прост, необратим и не ведет к потере качества на начальном этапе.

**Как это работает:**
Векторное изображение «переводится» в пиксельную сетку с заданным разрешением. Программа-конвертер вычисляет, какого цвета должен быть каждый пиксель, чтобы точно отобразить векторные объекты.

**Процесс:**
Экспорт / Сохранение как... → Выбор растрового формата (JPEG, PNG, etc.)

**Ключевой параметр:**
- **Разрешение (DPI)** - определяет конечное качество:
  - Для веба: 72-150 DPI
  - Для печати: 250-300 DPI и выше

**Результат:**
Полученное растровое изображение будет идеально четким *для своего размера*. Его дальнейшее увеличение будет приводить к пикселизации.

**Области применения:**
- Подготовка векторного логотипа для размещения на сайте
- Создание превью или финального рендера векторной иллюстрации
- Использование векторных элементов в виде текстур для 3D-моделирования

> **Вывод:** Конвертация вектора в растр — контролируемый и предсказуемый процесс, который используется на финальных стадиях работы для публикации или печати.

### 5.2. Конвертация растра в вектор (векторизация или трассировка)

Этот процесс сложен, часто неточен и требует значительных вычислительных ресурсов или ручной работы.

**Как это работает:**
Специальные алгоритмы (трассировщики) анализируют растровое изображение, пытаясь найти в массиве пикселей контуры и одноцветные области для описания математическими кривыми.

**Процесс:**
Открытие растрового изображения в векторном редакторе и использование функции «Image Trace» / «Трассировка изображения»

**Ключевые параметры:**
- **Сложность исходного изображения**
- **Настройки трассировки:** порог цвета, количество цветов, уровень детализации

**Результат в зависимости от сложности:**

| Тип изображения | Результат векторизации |
|----------------|------------------------|
| **Простое** (логотипы, клипарт) | Чистые, редактируемые контуры |
| **Сложное** (фотографии) | Стилизованное изображение с множеством избыточных форм |

**Области применения:**
- Восстановление логотипа из растровой копии низкого качества
- Создание векторной основы для доработки художником
- Подготовка изображений для лазерной гравировки или резки

> **Вывод:** Векторизация растра — не «волшебное» преобразование, а инструмент для решения конкретных задач, в основном связанных с получением контуров из простых растровых изображений.
